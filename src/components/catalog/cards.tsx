...
export function Pagination({ page, pageSize, total, basePath }: { page: number; pageSize: number; total: number; basePath: string }) {
  ...
  return (
    <nav aria-label="Pagination" className="mt-8 flex items-center justify-center gap-2">
      {page > 1 ? (
        <Link href={link(page - 1)} className="rounded-lg border border-sand-200 bg-white px-3 py-2 text-sm hover:border-brand-300">← Prev</Link>
      ) : null}
      <span className="px-2 text-sm text-ink-900/60">Page {page} of {pages}</span>
      {page < pages ? (
        <Link href={link(page + 1)} className="rounded-lg border border-sand-200 bg-white px-3 py-2 text-sm hover:border-brand-300">Next →</Link>
      ) : null}
    </nav>
  );
}
